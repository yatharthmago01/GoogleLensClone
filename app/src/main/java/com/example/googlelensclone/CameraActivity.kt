...
import kotlinx.android.synthetic.main.activity_main.btnTakePhoto
import java.io.File
...
class CameraActivity : AppCompatActivity() {
    ...
    private fun createNextPhoto(): Pair<Uri, OutputStream> {
        ...
        val fileName = "MLKitSample" + File.separator + displayName + ".jpg"

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            val contentValues = ContentValues().apply {
                put(MediaStore.MediaColumns.DISPLAY_NAME, displayName)
                put(MediaStore.MediaColumns.MIME_TYPE, "image/jpeg")
                put(MediaStore.MediaColumns.RELATIVE_PATH, Environment.DIRECTORY_DCIM + File.separator + fileName)
            }
    ...
    private fun takePhoto() {
        ...
        imageCapture!!.takePicture(
                ...
                override fun onError(exception: ImageCaptureException) {
                    Toast.makeText(this@CameraActivity, "Image Saving Failed", Toast.LENGTH_SHORT).show()
